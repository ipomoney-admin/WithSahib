...
const PatchSignalSchema = z.object({
  ...
  entry_low: z.number().optional(),
  entry_high: z.number().optional(),
})
...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  try {
    ...
    if (!user) {
      if (!rateLimit(`get-signal-public:${ip}`, 60, 60_000)) {
    ...
    logError('GET /api/signals/[id]', err)
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  try {
    ...
    const parsed = PatchSignalSchema.safeParse(rawBody)
    if (!parsed.success) {
      return NextResponse.json({ success: false, error: parsed.error.errors[0]?.message ?? 'Invalid request body' }, { status: 400 })
    }
    const { stop_loss, target_1, target_2, target_3, reason, entry_low, entry_high } = parsed.data
    ...
    // For intraday signals, modifications are only allowed during market hours
    if (signal.segment === 'intraday') {
      const marketOpen = await isMarketOpen()
      if (!marketOpen) {
        return NextResponse.json({ success: false, error: 'Intraday signal modifications are only allowed during market hours (9:00 AM – 3:30 PM IST, Mon–Fri).' }, { status: 400 })
      }
    }
    ...
    logError('PATCH /api/signals/[id]', err)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
